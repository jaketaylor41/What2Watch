...
function enabled() {
    ...
    $("#randomMovieBtn").on("click", function (event) {
        ...
        var settings = {
            ...
            "error": function (jqXHR, textStatus){
                if (jqXHR.status === 404) {
                    ...
                    $(".randomMovieContainer").append('<img id="movieImg" style="padding-top: 20px; height: 450px;" src="' + imgArray[rnd] + '"/>').hide().fadeIn(2000);
        ...
        $.ajax(settings).done(function (response) {
            console.log(response);
                $(".randomMovieContainer").append('<img id="movieImg" onerror="imgError(this)" style="padding-top: 20px; height: 450px;" src="http://image.tmdb.org/t/p/original' + response.poster_path + '"/>').hide().fadeIn(2000);
